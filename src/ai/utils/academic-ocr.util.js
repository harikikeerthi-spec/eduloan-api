...
exports.normalizeStateName = normalizeStateName;
exports.inferStateFromText = inferStateFromText;
exports.inferCityFromInstitution = inferCityFromInstitution;
exports.parseExamYear = parseExamYear;
exports.examYearToEndDate = examYearToEndDate;
exports.inferStartDate = inferStartDate;
exports.normalizeGradingSystem = normalizeGradingSystem;
exports.normalizeAcademicScore = normalizeAcademicScore;
exports.percentageFromTotalMarks = percentageFromTotalMarks;
exports.formatAcademicDate = formatAcademicDate;
exports.canonicalizeAcademicFields = canonicalizeAcademicFields;
const ocr_fields_util_1 = require("./ocr-fields.util");
const INDIAN_STATES = [
...
function normalizeStateName(stateStr) {
    ...
    if (clean === 'ap' || clean === 'andhra')
        return 'Andhra Pradesh';
    if (clean === 'ts' || clean === 'tel')
        return 'Telangana';
    if (clean === 'mh' || clean === 'maha')
        return 'Maharashtra';
    if (clean === 'up')
...
function inferStateFromText(...parts) {
    ...
    if (/\bandhra\s+pradesh\b/i.test(combined))
        return 'Andhra Pradesh';
    if (/\btelangana\b/i.test(combined))
        return 'Telangana';
    if (/\btamil\s+nadu\b/i.test(combined))
        return 'Tamil Nadu';
    if (/\buttar\s+pradesh\b/i.test(combined))
        return 'Uttar Pradesh';
    if (/\bwest\s+bengal\b/i.test(combined))
        return 'West Bengal';
    return undefined;
}
function inferCityFromInstitution(institution) {
    if (!institution)
        return undefined;
    ...
    const cityMatch = lastPart.match(/\b(REPALLE|GUNTUR|HYDERABAD|VIJAYAWADA|VISAKHAPATNAM|CHENNAI|MUMBAI|DELHI|BENGALURU|BANGALORE|PUNE|JAIPUR|LUCKNOW|KOLKATA|NAGPUR|INDORE|BHOPAL|COIMBATORE|MADURAI|SALEM|TRICHY|TIRUCHIRAPPALLI|KOCHI|THIRUVANANTHAPURAM|TRIVANDRUM|HUBLI|MYSORE|MYSURU|SURAT|AHMEDABAD|VADODARA|RAJKOT|NASHIK|AURANGABAD|MEERUT|AGRA|VARANASI|KANPUR|NOIDA|GHAZIABAD|FARIDABAD|GURGAON|GURUGRAM|CHANDIGARH|LUDHIANA|AMRITSAR|PATNA|RANCHI|RAIPUR|BHUBANESWAR|CUTTACK|GUWAHATI)\b/i);
    if (cityMatch) {
        const c = cityMatch[1];
...
function parseExamYear(examPeriod) {
    if (!examPeriod)
        return undefined;
    const match = String(examPeriod).match(/\b(19|20)\d{2}\b/);
    return match ? match[0] : undefined;
}
function examYearToEndDate(examPeriod) {
...
function inferStartDate(endDate, yearsBack = 2) {
...
function normalizeGradingSystem(grading, score) {
    const g = String(grading || '').toLowerCase();
    if (g.includes('cgpa') || g.includes('gpa') || g.includes('grade point'))
        return 'CGPA';
    if (g.includes('percent') || g.includes('%'))
        return 'Percentage';
    const s = String(score ?? '');
    if (/^\d(\.\d)?$/.test(s.trim()) && parseFloat(s) <= 10)
        return 'CGPA';
    if (parseFloat(s) > 10 && parseFloat(s) <= 100)
        return 'Percentage';
    return '';
}
function normalizeAcademicScore(score, grading) {
    if (score == null || score === '')
        return '';
    ...
    if (sys === 'Percentage')
        return num <= 100 ? String(Math.round(num * 10) / 10) : s;
    return s;
}
function percentageFromTotalMarks(secured, maximum) {
...
function yearsBackForLevel(level) {
    if (level === 'grade10')
        return 1;
    if (level === 'grade12')
        return 2;
    if (level === 'undergrad' || level === 'postgrad')
        return 3;
    return 2;
}
function canonicalizeAcademicFields(raw, level) {
    const out = {};
    const fullName = raw.full_name ||
        ...
        raw.name;
    if (fullName) {
        const cleaned = (0, ocr_fields_util_1.dedupeOcrFullName)(String(fullName));
        if (cleaned)
            out.full_name = cleaned;
    }
    const board = raw.board_name || raw.board || raw.examining_body;
    if (board)
        out.board = String(board).trim();
    const institution = raw.institution_name ||
        raw.institution ||
        raw.school_name ||
        raw.college_name;
    if (institution)
        out.institution = String(institution).trim();
    const university = raw.university_name ||
        raw.university ||
        (level !== 'grade10' && level !== 'grade12' ? institution : undefined);
    if (university)
        out.university = String(university).trim();
    const qualification = raw.qualification || raw.degree || raw.program_name || raw.course_name;
    if (qualification)
        out.qualification = String(qualification).trim();
    ...
    if (city)
        out.city = String(city).trim();
    const state = raw.state ||
        raw.state_of_study ||
        inferStateFromText(board, institution, raw.country);
    if (state)
        out.state = normalizeStateName(state);
    if (out.board) {
    ...
    if (country)
        out.country = String(country).trim();
    const language = raw.medium_of_instruction || raw.language || raw.medium;
    if (language)
        out.language = String(language).trim();
    const examPeriod = raw.examination_month_year ||
        raw.exam_month_year ||
        raw.exam_period ||
        raw.year_of_passing ||
        raw.passing_year;
    if (examPeriod)
        out.exam_period = String(examPeriod).trim();
    ...
    let grading = normalizeGradingSystem(raw.grading_system || raw.grading, score);
    let converted = false;
    ...
    if (grading)
        out.grading = grading;
    if (score != null && score !== '')
        out.score = normalizeAcademicScore(score, grading);
    ...
    if (certificateNumber)
        out.certificate_number = String(certificateNumber).trim();
    const barcodeNumber = raw.barcode_number || raw.barcode || raw.bar_code;
    if (barcodeNumber)
        out.barcode_number = String(barcodeNumber).trim();
    if (raw.father_name)
        out.father_name = String(raw.father_name).trim();
    if (raw.dob || raw.date_of_birth)
        out.dob = String(raw.dob || raw.date_of_birth).trim();
    return out;
}
//# sourceMappingURL=academic-ocr.util.js.map