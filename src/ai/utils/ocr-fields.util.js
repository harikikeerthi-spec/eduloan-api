...
exports.dedupeOcrFullName = dedupeOcrFullName;
exports.normalizeCountryName = normalizeCountryName;
exports.parsePlaceOfBirth = parsePlaceOfBirth;
exports.extractNameFromLabeledOcrText = extractNameFromLabeledOcrText;
exports.parseMRZLine1 = parseMRZLine1;
exports.parseMRZLine2 = parseMRZLine2;
exports.extractFullNameFromOcrRaw = extractFullNameFromOcrRaw;
exports.canonicalizeOcrFields = canonicalizeOcrFields;
exports.maskSensitiveIds = maskSensitiveIds;
const academic_ocr_util_1 = require("./academic-ocr.util");
...
function dedupeOcrFullName(raw) {
...
function normalizeGender(g) {
    if (!g || isPlaceholder(g))
        return undefined;
    const lower = String(g).trim().toLowerCase();
    if (lower === 'm' || lower.startsWith('m') || lower.includes('male') || lower.includes('पुरुष'))
        return 'male';
    if (lower === 'f' || lower.startsWith('f') || lower.includes('female') || lower.includes('महिला'))
        return 'female';
    if (lower.startsWith('o') || lower === 'other')
        return 'other';
    return undefined;
}
function normalizeCountryName(country) {
    ...
    if (!c)
        return '';
    if (c === 'ind' || c === 'indian' || c === 'india' || c.includes('india'))
        return 'India';
    if (c === 'usa' || c === 'us' || c.includes('united states'))
        return 'United States';
    if (c === 'uk' || c.includes('united kingdom'))
...
function parsePlaceOfBirth(pob) {
    if (!pob || isPlaceholder(pob))
        return {};
    ...
    if (parts.length >= 2) {
        ...
        const isLikelyCountry = last.length > 3 ||
...
function normalizeDobValue(dob) {
...
function normalizeDocTypeKey(docType) {
    ...
    if (d.includes('aadhaar') || d.includes('aadhar') || d.includes('national_id'))
        return 'aadhaar';
    if (d.includes('pan') && !d.includes('company'))
        return 'pan';
    if (d.includes('passport'))
        return 'passport';
    if (d.includes('marksheet_10') || d.includes('10th') || d.includes('ssc') || d.includes('grade_10') || d.includes('grade10'))
        return 'marksheet_10';
    if (d.includes('marksheet_12') || d.includes('12th') || d.includes('hsc') || d.includes('intermediate') || d.includes('grade_12') || d.includes('grade12'))
        return 'marksheet_12';
    if (d.includes('pg_degree') || d.includes('pg_transcript') || d.includes('marksheet_pg') || d.includes('postgraduate') || d.includes('post_grad'))
        return 'marksheet_pg';
    if (d.includes('ug_degree') || d.includes('ug_transcript') || d.includes('marksheet_ug') || d.includes('undergraduate') || d.includes('under_grad') || d.includes('bachelor'))
        return 'marksheet_ug';
    return 'generic';
}
function extractNameFromLabeledOcrText(text) {
    if (!text || isPlaceholder(text))
        return undefined;
    const clean = String(text).replace(/\r/g, '\n');
    const labelPatterns = [
        new RegExp(`(?:^|[\\n,;])\\s*(?:name|नाम)\\s*[:：]?\\s*(${OCR_NAME_WORD}(?:\\s+${OCR_NAME_WORD}){0,7})`, 'iu'),
        new RegExp(`(?:name|नाम)\\s*[:：]\\s*([^\\n,;]{2,80})`, 'iu'),
    ];
    for (const re of labelPatterns) {
        ...
        let candidate = m[1]
            .replace(/\s+/g, ' ')
            .replace(/\s+(?:dob|date\s*of\s*birth|year\s*of\s*birth|yob|gender|sex|male|female|जन्म|वर्ष|स्त्री|पुरुष).*$/iu, '')
            .trim();
        if (candidate.length >= 2 && !isPlaceholder(candidate)) {
            const deduped = dedupeOcrFullName(candidate);
            if (deduped && !isPlaceholder(deduped))
                return deduped;
        }
    }
    return undefined;
}
function parseMRZLine1(line, rawText) {
    ...
    if (rawText) {
        ...
        if (allWords.size > 0) {
            givenWords = givenWords.filter((w, idx) => {
                if (allWords.has(w))
                    return true;
                if (idx === givenWords.length - 1 && w.length <= 2)
                    return false;
                if (idx >= 2 && !allWords.has(w))
                    return false;
                return true;
            });
        }
...
function extractFullNameFromOcrRaw(raw, docType) {
    const kind = docType ? normalizeDocTypeKey(docType) : 'generic';
    const rawText = pickFirst(raw.raw_text_summary, raw.rawOcrText, raw.raw_text, raw.rawText);
    if (kind === 'passport') {
        if (rawText) {
        ...
        const sur = pickFirst(raw.surname, raw.last_name, raw.family_name, raw.lastName);
        const given = pickFirst(raw.given_names, raw.given_name, raw.first_name, raw.other_names, raw.firstName);
        if (given && sur) {
            const combined = dedupeOcrFullName(`${String(given).trim()} ${String(sur).trim()}`.replace(/\s+/g, ' '));
            return combined || undefined;
        }
    }
    const direct = pickFirst(raw.full_name, raw.fullName, raw.holder_name, raw.holderName, raw.cardholder_name, raw.cardholderName, raw.candidate_name, raw.candidateName, raw.student_name, raw.studentName, raw.applicant_name, raw.applicantName, raw.account_holder_name, raw.accountHolderName, raw.taxpayer_name, raw.taxpayerName, raw.person_name, raw.personName, raw.printed_name, raw.printedName, raw.name);
    if (direct) {
...
function academicLevelFromKind(kind) {
    if (kind === 'marksheet_10')
        return 'grade10';
    if (kind === 'marksheet_12')
        return 'grade12';
    if (kind === 'marksheet_ug')
        return 'undergrad';
    if (kind === 'marksheet_pg')
        return 'postgrad';
    return null;
}
function canonicalizeOcrFields(raw, docType) {
    if (!raw || typeof raw !== 'object')
        return {};
    const kind = normalizeDocTypeKey(docType);
    const out = {};
    const dob = pickFirst(raw.dob, raw.date_of_birth, raw.dateOfBirth, raw.birth_date, raw.year_of_birth, raw.yob);
    if (dob)
        out.dob = normalizeDobValue(String(dob).trim());
    const gender = normalizeGender(pickFirst(raw.gender, raw.sex));
    if (gender)
        out.gender = gender;
    if (kind === 'aadhaar') {
        const aadhaar = pickFirst(raw.aadhaar_number, raw.aadhaarNumber, raw.aadhar_number, raw.document_number, raw.national_id_number);
        if (aadhaar)
            out.aadhaar_number = String(aadhaar).replace(/\s+/g, ' ').trim();
        const vid = pickFirst(raw.vid, raw.VID);
        if (vid)
            out.vid = String(vid).trim();
        const address = raw.address;
        if (address && typeof address === 'object' && !Array.isArray(address)) {
            const addr = {};
            for (const [k, v] of Object.entries(address)) {
                if (v != null && String(v).trim())
                    addr[k] = String(v).trim();
            }
            if (Object.keys(addr).length > 0) {
                out.address = addr;
                if (addr.pincode)
                    out.pin_code = addr.pincode;
            }
        }
        else {
            const addrStr = pickFirst(raw.address_formatted, typeof raw.address === 'string' ? raw.address : undefined, raw.permanent_address, raw.permanentAddress);
            if (addrStr)
                out.address = String(addrStr).trim();
        }
        const pin = pickFirst(raw.pin_code, raw.pincode, raw.zip);
        if (pin && !out.pin_code)
            out.pin_code = String(pin).trim();
    }
    else if (kind === 'pan') {
        const pan = pickFirst(raw.pan_number, raw.panNumber, raw.document_number, raw.pan);
        if (pan)
            out.pan_number = String(pan).toUpperCase().trim();
        const father = pickFirst(raw.father_name, raw.fatherName, raw.guardian_name);
        if (father)
            out.father_name = dedupeOcrFullName(String(father));
        const country = pickFirst(raw.country, raw.nationality);
        if (country)
            out.country = normalizeCountryName(String(country));
        const authority = pickFirst(raw.authority, raw.issuing_authority, raw.issuingAuthority);
        if (authority)
            out.authority = String(authority).trim();
        const government = pickFirst(raw.government, raw.government_name);
        if (government)
            out.government = String(government).trim();
        for (const flag of ['signature_present', 'photo_present', 'qr_code_present']) {
            if (raw[flag] === true || raw[flag] === false)
                out[flag] = raw[flag];
        }
    }
        ...
        const passportNum = pickFirst(raw.passport_number, raw.passportNumber, raw.document_number, mrzData.passport_number);
        if (passportNum)
            out.passport_number = String(passportNum).trim();
        const given = pickFirst(raw.given_names, raw.given_name, raw.first_name, raw.firstName, mrzData.given_names);
        if (given)
            out.given_names = String(given).trim();
        const sur = pickFirst(raw.surname, raw.last_name, raw.lastName, mrzData.surname);
        if (sur)
            out.surname = String(sur).trim();
        const nationality = pickFirst(raw.nationality, raw.citizenship);
        if (nationality)
            out.nationality = String(nationality).trim();
        const issueDate = pickFirst(raw.date_of_issue, raw.issue_date, raw.issueDate);
        if (issueDate)
            out.date_of_issue = normalizeDobValue(String(issueDate).trim());
        const expiry = pickFirst(raw.date_of_expiry, raw.expiry_date, raw.expiryDate, mrzData.date_of_expiry);
        if (expiry)
            out.date_of_expiry = normalizeDobValue(String(expiry).trim());
        const dob = pickFirst(out.dob, mrzData.dob);
        if (dob)
            out.dob = normalizeDobValue(String(dob).trim());
        const gender = normalizeGender(pickFirst(out.gender, mrzData.gender));
        if (gender)
            out.gender = gender;
        const issueCountry = pickFirst(raw.issue_country, raw.issueCountry, raw.country_of_issue, raw.issuing_country);
        if (issueCountry)
            out.issue_country = normalizeCountryName(String(issueCountry));
        const poi = pickFirst(raw.place_of_issue, raw.issue_place, raw.issuing_authority);
        if (poi)
            out.place_of_issue = String(poi).trim();
        const birthCity = pickFirst(raw.birth_city, raw.birthCity, raw.city_of_birth);
        if (birthCity)
            out.birth_city = String(birthCity).trim();
        const birthCountry = pickFirst(raw.birth_country, raw.birthCountry, raw.country_of_birth);
        if (birthCountry) {
        ...
        if (pob) {
            ...
            if (!out.birth_city || !out.birth_country) {
                ...
                if (parsed.birth_city && !out.birth_city)
                    out.birth_city = parsed.birth_city;
                if (parsed.birth_country && !out.birth_country)
                    out.birth_country = parsed.birth_country;
            }
        }
        if (!out.birth_country && out.nationality) {
            const nat = String(out.nationality).toLowerCase();
            if (nat.includes('indian') || nat === 'ind')
                out.birth_country = 'India';
        }
        if (!out.issue_country && out.nationality) {
            const nat = String(out.nationality).toLowerCase();
            if (nat.includes('indian') || nat === 'ind')
                out.issue_country = 'India';
        }
        ...
        if (addr)
            out.address = typeof addr === 'object' ? addr : String(addr).trim();
    }
    else if (kind === 'marksheet_10' || kind === 'marksheet_12' || kind === 'marksheet_ug' || kind === 'marksheet_pg') {
        const level = academicLevelFromKind(kind);
        Object.assign(out, (0, academic_ocr_util_1.canonicalizeAcademicFields)(raw, level));
    }
    else {
        const skip = new Set([
            'date_of_birth',
    ...
    if (resolvedName)
        out.full_name = resolvedName;
    return out;
}
function maskSensitiveIds(data, docType) {
    const masked = { ...data };
    const kind = normalizeDocTypeKey(docType);
...
//# sourceMappingURL=ocr-fields.util.js.map