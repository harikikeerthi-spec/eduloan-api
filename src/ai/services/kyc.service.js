...
const ocr_fields_util_1 = require("../utils/ocr-fields.util");
const pan_validation_util_1 = require("../utils/pan-validation.util");
...
let KycService = class KycService {
    ...
    async detectDocumentType(buffer, mimetype) {
        ...
        try {
            const response = await this.openRouterService.chatWithVision(prompt, `data:${mimetype};base64,${base64}`, 'google/gemini-2.5-flash');
    ...
    async processDocument(buffer, mimetype, expectedType) {
        ...
        if (!isImage && !isPdf) {
            ...
            const dynamicData = (0, ocr_fields_util_1.canonicalizeOcrFields)(this.extractDynamicFieldsFromText(text, docType), docType);
            return {
        ...
        try {
            ...
            const dynamicData = (0, ocr_fields_util_1.canonicalizeOcrFields)({
                ...rawDynamic,
            ...
            const needsName = lowerType.includes('aadhaar') ||
    ...
    async validateDocumentKeywords(buffer, docType, isPdf, isImage) {
    ...
    logAudit(type, confidence, isValid, error) {
    ...
    parseAiResponse(response, docType) {
        try {
            ...
            const extracted = (0, ocr_fields_util_1.canonicalizeOcrFields)({
                ...rawExtracted,
            ...
            if (!extracted.full_name) {
                const fromSummary = (0, ocr_fields_util_1.extractNameFromLabeledOcrText)(String(parsed.raw_text_summary || parsed.rawOcrText || ''));
                if (fromSummary)
                    extracted.full_name = fromSummary;
            }
            const lowerType = String(docType || '').toLowerCase();
            ...
            if (parsed.fraud_reason === 'WRONG_DOCUMENT_TYPE_UPLOADED') {
                ...
                if (expectedNorm !== detectedNorm) {
                    ...
                    if (isAadhaar) {
                        ...
                        const panValidation = (0, pan_validation_util_1.validatePanExtraction)(extracted, parsed);
            ...
            return {
                document_type: (parsed.document_type || docType).toLowerCase(),
    ...
    normalizeDocTypeForComparison(type) {
    ...
    validateAadhaarDocument(_parsed, extracted) {
        const failedLabels = [];
        if (!extracted.full_name) {
    ...
    extractDynamicFieldsFromText(text, docType) {
        ...
        if (normalizedType.includes('aadhaar') || normalizedType.includes('aadhar') || normalizedType.includes('national_id')) {
            ...
            let foundName = (0, ocr_fields_util_1.extractNameFromLabeledOcrText)(clean) || '';
            if (!foundName) {
                const skipKeywords = [
            ...
            if (addressStartIdx !== -1) {
                ...
                if (finalAddress.length > 10) {
                    ...
                    if (addressParts.length > 0)
                        structuredAddress.house_details = addressParts[0];
                    if (addressParts.length > 1)
                        structuredAddress.area = addressParts[1];
                    if (addressParts.length > 2)
                        structuredAddress.landmark = addressParts[2];
                    if (addressParts.length > 3)
                        structuredAddress.mandal = addressParts[3];
                    if (addressParts.length > 4)
                        structuredAddress.district = addressParts[4];
                    if (foundState)
                        structuredAddress.state = foundState;
                    if (data.pin_code)
                        structuredAddress.pincode = data.pin_code;
            ...
            const labeledPanName = (0, ocr_fields_util_1.extractNameFromLabeledOcrText)(clean);
            if (labeledPanName) {
            ...
            if (allDates.length >= 1)
                data.dob = allDates[0][0];
            if (allDates.length >= 2)
                data.date_of_issue = allDates[1][0];
            if (allDates.length >= 3)
                data.date_of_expiry = allDates[2][0];
            else if (allDates.length === 2)
                data.date_of_expiry = allDates[1][0];
            if (/\bfemale\b/i.test(clean))
                data.gender = 'female';
            else if (/\bmale\b/i.test(clean))
                data.gender = 'male';
            ...
            if (pobMatch)
                data.place_of_birth = pobMatch[1].trim();
            const poiMatch = clean.match(/place\s*of\s*issue[:\s]+([^\n]+)/i);
            if (poiMatch)
                data.place_of_issue = poiMatch[1].trim();
            const labeledPassportName = (0, ocr_fields_util_1.extractNameFromLabeledOcrText)(clean);
            ...
            if (!data.full_name) {
                ...
                if (nameLine)
                    data.full_name = nameLine.replace(/\s+/g, ' ').trim();
            }
            const pinMatch = clean.match(/\b\d{6}\b/);
            if (pinMatch) {
