...
function getDocumentValidationSchema(normalizedType) {
...
function getExtractedDataSchema(normalizedType) {
    if (normalizedType === 'aadhaar') {
        return `{
...
function buildOcrPrompt(docType, studentProfile) {
...
function normalizeDocType(docType) {
    ...
    if (d.includes('aadhaar') || d.includes('aadhar') || d.includes('adhar'))
        return 'aadhaar';
    if (d.includes('pan') && !d.includes('company'))
        return 'pan';
    if (d.includes('passport'))
        return 'passport';
    if (d.includes('admission') || d.includes('offer') || d.includes('i20') || d.includes('i_20') || d.includes('cas'))
        return 'admission_letter';
    if (d.includes('bank_statement') || d.includes('bank_stmt'))
        return 'bank_statement';
    if (d.includes('itr') || d.includes('income_tax'))
        return 'itr';
    ...
    if (d.includes('ug_degree') || d.includes('ug_transcript') || d.includes('marksheet_ug') || d.includes('undergraduate') || d.includes('under_grad') || d.includes('bachelor'))
        return 'marksheet_ug';
    if (d.includes('marksheet') || d.includes('transcript'))
        return 'marksheet';
    return 'generic';
}
let DocumentVerificationService = class DocumentVerificationService {
    openRouterService;
    constructor(openRouterService) {
        this.openRouterService = openRouterService;
    }
    async verifyAndExtractDocument(docType, fileBuffer, mimetype, studentProfile) {
        ...
        try {
            ...
            const extracted = (0, ocr_fields_util_1.canonicalizeOcrFields)(rawExtracted, docType);
            const normalizedType = normalizeDocType(docType);
            ...
            if (normalizedType === 'aadhaar') {
                ...
                const panCheck = (0, pan_validation_util_1.validatePanExtraction)(extracted, parsed);
    ...
    async reverifyDocumentForAdmin(fileBuffer, mimetype, docType, studentProfile) {
    ...
    normalizeAadhaarExtractedFields(extracted) {
    ...
    validateAadhaarExtraction(_parsed, extracted) {
        const failed = [];
    ...
    namesMatch(profileName, documentName) {
    ...
    fallbackVerification(docType, rawText) {
    ...
    async explainRejection(docType, reason) {
        const prompt = `
...
exports.DocumentVerificationService = DocumentVerificationService = __decorate([
