...
let StaffProfileService = class StaffProfileService {
    ...
    constructor(supabase, usersService, auditLog, s3Service, eventEmitter, emailService) {
    ...
    async createProfile(staffUser, body) {
        ...
        if (body.target_bank)
            insertData.targetBank = body.target_bank;
        if (body.loan_type)
            insertData.loanType = body.loan_type;
        ...
        if (error) {
            ...
            throw new common_1.BadRequestException(`Failed to create staff profile: ${error.message || 'Database error'}`);
        }
    ...
    async listProfiles(staffUser, query) {
        let q = this.db
            .from('StaffProfile')
        ...
        if (query.search) {
            ...
            results = results.filter((p) => p.linkedUser?.firstName?.toLowerCase().includes(s) ||
    ...
    async getProfile(profileId) {
        const { data, error } = await this.db
            .from('StaffProfile')
            .select(`*, linkedUser:User!linkedUserId(id, firstName, lastName, email, mobile, phoneNumber, dateOfBirth),
    ...
    async getProfileByLinkedUserId(linkedUserId) {
        const { data, error } = await this.db
            .from('StaffProfile')
            .select(`*, linkedUser:User!linkedUserId(id, firstName, lastName, email, mobile, phoneNumber, dateOfBirth)`)
    ...
    async uploadStaffDocument(profileId, staffUser, file, body) {
        if (!file)
            throw new common_1.BadRequestException('File is required');
        const profile = await this.getProfile(profileId);
        const linkedUserId = profile.linkedUserId || profileId;
        const s3Key = this.s3Service.buildKey(linkedUserId, body.doc_type, file.originalname);
    ...
    async updateDocumentStatus(profileId, docId, staffUser, body) {
    ...
    async shareWithBank(profileId, staffUser, body) {
        ...
        const token = Buffer.from(JSON.stringify({ profileId, ts: Date.now(), r: Math.random() })).toString('base64url');
    ...
    async getShareHistory(profileId) {
        const { data, error } = await this.db
            .from('StaffProfileShare')
            .select('*')
            .eq('staffProfileId', profileId)
    ...
    async removeDocument(profileId, docId, staffUser) {
    ...
    async logDashboardActivity(user, data) {
    ...
    async getDashboardActivities(limit = 15) {
        const { data, error } = await this.db
            .from('AuditLog')
        ...
        if (error)
            throw error;
        return (data || []).map((row) => this.formatDashboardActivity(row));
    }
    async getAllDashboardActivities(opts) {
        ...
        if (error)
            throw error;
        return {
            items: (data || []).map((row) => this.formatDashboardActivity(row)),
            total: count || 0,
        };
    }
    ...
    mapOnboardingToApplication(onboarding) {
        if (!onboarding)
            return {};
        ...
        let coApplicantIncome = null;
        if (onboarding.coApplicant?.monthlyIncome) {
            coApplicantIncome = parseFloat(onboarding.coApplicant.monthlyIncome) * 12;
        }
    ...
    async shareProfile(studentId, staffUser, body) {
        if (!studentId)
        ...
        if (!profile) {
            ...
            if (createProfileErr) {
                ...
                throw new common_1.BadRequestException(`Failed to create StaffProfile: ${createProfileErr.message}`);
            }
            profile = newProfile;
        }
        else {
        ...
        if (documentIds.length === 0) {
            ...
            if (userDocs && userDocs.length > 0) {
                ...
                const { data: insertedDocs } = await this.db
                    ...
                    .select();
                if (insertedDocs) {
                    insertedDocs.forEach((d) => documentIds.push(d.id));
                }
            }
        }
        const expiresAt = new Date();
        expiresAt.setDate(expiresAt.getDate() + 30);
        const token = Buffer.from(JSON.stringify({ profileId: profile.id, ts: Date.now(), r: Math.random() })).toString('base64url');
        ...
        if (shareErr) {
            ...
            throw new common_1.BadRequestException(`Failed to create StaffProfileShare: ${shareErr.message}`);
        }
      ...
      `;
            ...
            if (!application) {
                ...
                if (createAppErr) {
                    ...
                    throw new common_1.BadRequestException(`Failed to create LoanApplication: ${createAppErr.message}`);
                }
                application = newApp;
    ...
    async getAiPredictionScore(applicationId) {
