...
let FirebaseAuthService = class FirebaseAuthService {
    ...
    constructor(configService) {
        ...
        if (!admin.apps.length) {
            ...
            if (!projectId || !clientEmail || !privateKey) {
                console.warn('[FirebaseAuthService] Firebase credentials missing in .env. Firebase Authentication will be disabled.');
                return;
            }
            try {
    ...
    async verifyToken(idToken) {
        if (!this.isEnabled()) {
            throw new common_1.UnauthorizedException('Firebase Authentication is currently disabled (missing server credentials)');
        }
        try {
