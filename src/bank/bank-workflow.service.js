...
exports.BankWorkflowService = void 0;
...
let BankWorkflowService = class BankWorkflowService {
    db;
    eventEmitter;
    ...
    async submitApplicationToBank(applicationId, bankId, bankName, submittedBy) {
        ...
        const { data: existing } = await this.db.client
            ...
            .single();
        if (existing) {
            throw new common_1.BadRequestException('Application already submitted to this bank');
        }
    ...
    async logFile(submissionId, lanNumber, loggedBy, notes) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'FILE_LOGGED')) {
            throw new common_1.BadRequestException(`Cannot transition from ${submission.workflowStatus} to FILE_LOGGED`);
        }
        const { data: existingLan } = await this.db.client
            ...
            .single();
        if (existingLan) {
            throw new common_1.BadRequestException('LAN number already exists');
        }
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, 'SUBMITTED_TO_BANK', 'FILE_LOGGED', loggedBy, `File logged with LAN: ${lanNumber}`);
        await this.db.client
            .from('LoanApplication')
            .update({
    ...
    async moveToUnderReview(submissionId, changedBy, notes) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'UNDER_REVIEW')) {
            throw new common_1.BadRequestException(`Cannot transition from ${submission.workflowStatus} to UNDER_REVIEW`);
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'UNDER_REVIEW', changedBy, notes || 'Application moved to under review');
        await this.db.client
            .from('LoanApplication')
    ...
    async raiseQuery(submissionId, queryType, queryDescription, raisedBy, dueDate, docsChecklist = [], attachments = []) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'QUERY_RAISED')) {
            throw new common_1.BadRequestException(`Cannot raise query from ${submission.workflowStatus} status`);
        }
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'QUERY_RAISED', raisedBy, `Query raised: ${queryType} - ${queryDescription}`);
        await this.db.client
            .from('LoanApplication')
    ...
    async respondToQuery(queryId, response, respondedBy, attachments = [], docsChecklist = []) {
        const { data: query, error: queryError } = await this.db.client
            .from('BankWorkflowQueryRequest')
            .select('*')
            .eq('id', queryId)
        ...
        if (!pendingQueries || pendingQueries.length === 0) {
            ...
            await this.db.client
                ...
                .eq('id', query.submissionId);
            await this.recordWorkflowHistory(query.submissionId, query.applicationId, 'QUERY_RAISED', 'UNDER_REVIEW', respondedBy, 'All queries responded, moving back to under review');
            await this.db.client
                .from('LoanApplication')
                .update({ bankWorkflowStatus: 'UNDER_REVIEW' })
    ...
    async sanctionApplication(submissionId, sanctionDetails, decidedBy) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'SANCTIONED')) {
            throw new common_1.BadRequestException(`Cannot sanction from ${submission.workflowStatus} status`);
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'SANCTIONED', decidedBy, `Application sanctioned for ₹${sanctionDetails.sanctionAmount}`);
        await this.db.client
            .from('LoanApplication')
            .update({
    ...
    async conditionalSanctionApplication(submissionId, sanctionDetails, decidedBy) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'CONDITIONAL_SANCTION')) {
            throw new common_1.BadRequestException(`Cannot conditionally sanction from ${submission.workflowStatus} status`);
        }
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'CONDITIONAL_SANCTION', decidedBy, `Conditional sanction for ₹${sanctionDetails.sanctionAmount} with ${structuredConditions.length} condition(s)`);
        await this.db.client
            .from('LoanApplication')
    ...
    async updateConditionStatus(submissionId, conditionIndex, status, updatedBy) {
        const VALID_CONDITION_STATUSES = ['PENDING', 'MET', 'WAIVED'];
        if (!VALID_CONDITION_STATUSES.includes(status)) {
            throw new common_1.BadRequestException(`Invalid condition status "${status}". Allowed: ${VALID_CONDITION_STATUSES.join(', ')}`);
        }
        const submission = await this.getSubmission(submissionId);
        if (submission.workflowStatus !== 'CONDITIONAL_SANCTION') {
            throw new common_1.BadRequestException('Submission is not in CONDITIONAL_SANCTION status.');
        }
        const conditions = submission.conditions || [];
        const condIdx = conditions.findIndex((c) => c.index === conditionIndex);
        if (condIdx === -1) {
            throw new common_1.NotFoundException(`Condition at index ${conditionIndex} not found.`);
        }
        ...
        if (error)
            throw error;
        const allMet = conditions.every((c) => c.status === 'MET' || c.status === 'WAIVED');
        if (allMet) {
    ...
    async acceptCounterOffer(submissionId, acceptedBy) {
        const submission = await this.getSubmission(submissionId);
        if (submission.workflowStatus !== 'COUNTER_OFFER') {
            throw new common_1.BadRequestException('Submission is not in COUNTER_OFFER status.');
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, 'COUNTER_OFFER', 'SANCTIONED', acceptedBy, 'Counter offer accepted by applicant/co-applicant');
        await this.db.client
            .from('LoanApplication')
    ...
    async rejectCounterOffer(submissionId, reason, rejectedBy) {
        const submission = await this.getSubmission(submissionId);
        if (submission.workflowStatus !== 'COUNTER_OFFER') {
            throw new common_1.BadRequestException('Submission is not in COUNTER_OFFER status.');
        }
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, 'COUNTER_OFFER', 'REJECTED', rejectedBy, `Counter offer rejected: ${reason}`);
        await this.db.client
            .from('LoanApplication')
    ...
    async partialSanctionApplication(submissionId, details, decidedBy) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'CONDITIONAL_SANCTION')) {
            throw new common_1.BadRequestException(`Cannot issue partial sanction from ${submission.workflowStatus} status`);
        }
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'PARTIAL_SANCTION', decidedBy, `Partial sanction: Approved ₹${details.approvedAmount} of ₹${details.requestedAmount} requested (shortfall ₹${shortfallAmount})`);
        await this.db.client
            .from('LoanApplication')
    ...
    async makeCounterOffer(submissionId, counterOfferDetails, decidedBy) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'COUNTER_OFFER')) {
            throw new common_1.BadRequestException(`Cannot make counter offer from ${submission.workflowStatus} status`);
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'COUNTER_OFFER', decidedBy, `Counter offer made: ₹${counterOfferDetails.sanctionAmount} @ ${counterOfferDetails.roiEffective}%`);
        await this.db.client
            .from('LoanApplication')
    ...
    async rejectApplication(submissionId, rejectionDetails, decidedBy) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'REJECTED')) {
            throw new common_1.BadRequestException(`Cannot reject from ${submission.workflowStatus} status`);
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'REJECTED', decidedBy, `Rejected: ${rejectionDetails.category} - ${rejectionDetails.reason}`);
        await this.db.client
            .from('LoanApplication')
            .update({
    ...
    async moveToProcessingFee(submissionId, feeAmount, changedBy) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'PROCESSING_FEE')) {
            throw new common_1.BadRequestException(`Cannot move to PROCESSING_FEE from ${submission.workflowStatus} status`);
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'PROCESSING_FEE', changedBy, `Processing fee of ₹${feeAmount} pending`);
        await this.db.client
            .from('LoanApplication')
    ...
    async markFeeAsPaid(submissionId, changedBy) {
        const submission = await this.getSubmission(submissionId);
        if (submission.workflowStatus !== 'PROCESSING_FEE') {
            throw new common_1.BadRequestException('Submission not in PROCESSING_FEE status');
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, 'PROCESSING_FEE', 'DISBURSEMENT_PENDING', changedBy, 'Processing fee paid, moving to disbursement');
        await this.db.client
            .from('LoanApplication')
    ...
    async confirmDisbursement(submissionId, disbursementDetails, confirmedBy) {
        const submission = await this.getSubmission(submissionId);
        if (!this.isValidTransition(submission.workflowStatus, 'DISBURSED')) {
            throw new common_1.BadRequestException(`Cannot disburse from ${submission.workflowStatus} status`);
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'DISBURSED', confirmedBy, `Disbursement confirmed: ₹${disbursementDetails.amount} (Ref: ${disbursementDetails.referenceNo})`);
        await this.db.client
            .from('LoanApplication')
            .update({
    ...
    async allowResubmission(submissionId, authorizedBy) {
        const submission = await this.getSubmission(submissionId);
        if (submission.workflowStatus !== 'REJECTED') {
            throw new common_1.BadRequestException('Only rejected applications can be resubmitted');
        }
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, 'REJECTED', 'RESUBMIT_OTHER_BANK', authorizedBy, 'Application allowed to be resubmitted to another bank');
        return { success: true, data: updated };
    }
    async getSubmission(submissionId) {
        const { data: submission, error } = await this.db.client
            ...
            .single();
        if (error || !submission) {
            throw new common_1.NotFoundException('Bank submission not found');
        }
        return submission;
    }
    ...
    async recordWorkflowHistory(submissionId, applicationId, fromStatus, toStatus, changedBy, changeReason) {
        await this.db.client
    ...
    async getBankIncomingApplications(bankId, filters) {
    ...
    async getBankWorkflowAnalytics(bankId) {
        ...
        const byDecision = submissions.reduce((acc, s) => {
            if (s.decisionStatus) {
    ...
    async addQueryMessage(queryId, message, sender, attachments = []) {
        const { data: query, error: queryError } = await this.db.client
            .from('BankWorkflowQueryRequest')
            .select('*')
            .eq('id', queryId)
    ...
    async getQueryTemplatesByBank(bankId) {
    ...
    async updateQueryTemplate(templateId, data) {
        const { data: template, error } = await this.db.client
            .from('BankQueryTemplate')
            .update({
            ...data,
    ...
    async setSubmissionHold(submissionId, reason, changedBy) {
        const submission = await this.getSubmission(submissionId);
        if (submission.isOnHold) {
            throw new common_1.BadRequestException('Submission is already on hold');
        }
        const now = new Date().toISOString();
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, submission.workflowStatus, changedBy, `Submission placed on hold. Reason: ${reason}`);
        const notifData = {
    ...
    async resumeSubmissionHold(submissionId, changedBy) {
        const submission = await this.getSubmission(submissionId);
        if (!submission.isOnHold) {
            throw new common_1.BadRequestException('Submission is not on hold');
        }
        const now = new Date();
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, submission.workflowStatus, changedBy, `Submission hold resumed. Paused duration: ${Math.round(holdDurationMs / 1000 / 60)} minutes.`);
        const notifData = {
            id: 'notif-' + Date.now() + '-' + Math.floor(Math.random() * 1000),
            userId: 'staff',
    ...
    async bulkTransferSubmissions(submissionIds, officerId, officerName, changedBy) {
        const results = [];
        for (const id of submissionIds) {
            ...
            if (error)
                throw error;
            await this.recordWorkflowHistory(id, submission.applicationId, submission.workflowStatus, submission.workflowStatus, changedBy, `Assigned officer updated to: ${officerName}`);
            const notifData = {
                id: 'notif-' + Date.now() + '-' + Math.floor(Math.random() * 1000),
                userId: officerId,
    ...
    async updateFeeStatus(submissionId, status, paymentRef, changedBy) {
        const submission = await this.getSubmission(submissionId);
        const updatePayload = {
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, submission.workflowStatus, changedBy || 'system', `Processing fee status updated to: ${status} ${paymentRef ? '(Ref: ' + paymentRef + ')' : ''}`);
        return { success: true, data: updated };
    }
    async scheduleDisbursementTranche(submissionId, amount, dueDate, remarks) {
        const submission = await this.getSubmission(submissionId);
        if (submission.workflowStatus !== 'DISBURSEMENT_PENDING' && submission.workflowStatus !== 'DISBURSED') {
            throw new common_1.BadRequestException('Disbursements can only be scheduled in DISBURSEMENT_PENDING or DISBURSED stages.');
        }
        const sanctionAmount = submission.sanctionAmount || 0;
        const tranches = submission.disbursementTranches || [];
        const totalTranchesAmt = tranches
            .filter((t) => t.status !== 'CANCELLED')
            .reduce((sum, t) => sum + (t.amount || 0), 0);
        const remainingAmount = sanctionAmount - totalTranchesAmt;
        if (amount > remainingAmount) {
            throw new common_1.BadRequestException(`Requested tranche amount of ₹${amount} exceeds the remaining un-scheduled sanction amount of ₹${remainingAmount}`);
        }
    ...
    async confirmDisbursementTranche(submissionId, trancheNumber, referenceNo, confirmedBy) {
        const submission = await this.getSubmission(submissionId);
        const tranches = submission.disbursementTranches || [];
        const trancheIdx = tranches.findIndex((t) => t.trancheNumber === trancheNumber);
        if (trancheIdx === -1) {
            throw new common_1.NotFoundException(`Tranche ${trancheNumber} not found.`);
        }
        const targetTranche = tranches[trancheIdx];
        if (targetTranche.status === 'COMPLETED') {
            throw new common_1.BadRequestException(`Tranche ${trancheNumber} is already completed.`);
        }
        for (let i = 1; i < trancheNumber; i++) {
            const prev = tranches.find((t) => t.trancheNumber === i);
            if (prev && prev.status === 'PENDING') {
                throw new common_1.BadRequestException(`Tranche sequencing violation: Tranche ${i} must be completed or cancelled before confirming Tranche ${trancheNumber}`);
            }
        }
        tranches[trancheIdx] = {
            ...targetTranche,
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, 'DISBURSED', confirmedBy, `Disbursement tranche ${trancheNumber} of ₹${targetTranche.amount} confirmed (Ref: ${referenceNo})`);
        await this.db.client
            .from('LoanApplication')
            .update({
            bankWorkflowStatus: 'DISBURSED',
    ...
    async getTranchesSummary(submissionId) {
        const submission = await this.getSubmission(submissionId);
        const tranches = submission.disbursementTranches || [];
        const sanctionAmount = submission.sanctionAmount || 0;
    ...
    async amendSanctionTerms(submissionId, newTerms, reason, effectiveDate, amendedBy) {
        const submission = await this.getSubmission(submissionId);
        if (submission.workflowStatus !== 'SANCTIONED' && submission.workflowStatus !== 'PROCESSING_FEE' && submission.workflowStatus !== 'DISBURSEMENT_PENDING' && submission.workflowStatus !== 'DISBURSED') {
            throw new common_1.BadRequestException('Amendments can only be filed after initial sanction is issued.');
        }
        const originalTerms = {
        ...
        if (Object.keys(diff).length === 0) {
            throw new common_1.BadRequestException('No terms were altered. Amendment skipped.');
        }
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, submission.workflowStatus, amendedBy, `Sanction terms amended: ${reason}. Diff: ${JSON.stringify(diff)}`);
        const appUpdate = {};
        if (newTerms.sanctionAmount !== undefined)
            appUpdate.sanctionAmount = newTerms.sanctionAmount;
        if (newTerms.roiEffective !== undefined)
            appUpdate.roiEffective = newTerms.roiEffective;
        await this.db.client
            .from('LoanApplication')
            .update(appUpdate)
            .eq('id', submission.applicationId);
    ...
    async requestCancellation(submissionId, reason, requestedBy) {
        const submission = await this.getSubmission(submissionId);
        const { data: updated, error } = await this.db.client
            .from('BankSubmission')
            .update({
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, submission.workflowStatus, submission.workflowStatus, requestedBy, `Cancellation requested. Reason: ${reason}`);
        return { success: true, data: updated };
    }
    async confirmCancellation(submissionId, confirmedBy) {
        const submission = await this.getSubmission(submissionId);
        ...
        if (error)
            throw error;
        await this.recordWorkflowHistory(submissionId, submission.applicationId, stage, 'REJECTED', confirmedBy, `Cancellation confirmed. Stage: ${stage}. Fee: ₹${feeAmount}, refund: ${refundPercent}% (₹${calculatedRefund})`);
        await this.db.client
            .from('LoanApplication')
            .update({
            bankWorkflowStatus: 'REJECTED',
    ...
    async submitQualityRating(submissionId, ratings, comments, ratedBy) {
        const submission = await this.getSubmission(submissionId);
        const { documentation, credit, profile, communication } = ratings;
        if (documentation < 1 || documentation > 5 ||
            credit < 1 || credit > 5 ||
            profile < 1 || profile > 5 ||
            communication < 1 || communication > 5) {
            throw new common_1.BadRequestException('Ratings must be between 1 and 5 stars for all dimensions.');
        }
    ...
    async getCrossBankHistory(submissionId) {
        ...
        const { data: application, error: appError } = await this.db.client
            ...
            .single();
        if (appError || !application) {
            throw new common_1.NotFoundException('Parent loan application not found.');
        }
    ...
    async grantStudentConsent(studentId, bankId, isGranted, ipAddress, userAgent) {
    ...
    async verifyStudentConsent(studentId, bankId) {
        const { data, error } = await this.db.client
            .from('StudentBankConsent')
            .select('isGranted')
    ...
    async getPipelineFunnelAnalytics(bankId) {
        let query = this.db.client
            .from('BankSubmission')
        ...
        (submissions || []).forEach((s) => {
            ...
            if (status === 'SUBMITTED_TO_BANK')
                funnelStages.SUBMITTED_TO_BANK++;
            else if (status === 'FILE_LOGGED')
                funnelStages.FILE_LOGGED++;
            else if (status === 'UNDER_REVIEW')
                funnelStages.UNDER_REVIEW++;
            else if (status === 'QUERY_RAISED')
                funnelStages.QUERY_RAISED++;
            else if (status === 'SANCTIONED' || status === 'CONDITIONAL_SANCTION' || status === 'COUNTER_OFFER')
                funnelStages.SANCTIONED++;
            else if (status === 'PROCESSING_FEE' || status === 'DISBURSEMENT_PENDING')
                funnelStages.DISBURSEMENT_PENDING++;
            else if (status === 'DISBURSED')
                funnelStages.DISBURSED++;
            else if (status === 'REJECTED')
                funnelStages.CANCELLED_OR_REJECTED++;
        });
        return {
            success: true,
            bankId: bankId || 'all',
...
exports.BankWorkflowService = BankWorkflowService;
exports.BankWorkflowService = BankWorkflowService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [supabase_service_1.SupabaseService,
        event_emitter_1.EventEmitter2])
], BankWorkflowService);
//# sourceMappingURL=bank-workflow.service.js.map