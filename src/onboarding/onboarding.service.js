...
let OnboardingService = class OnboardingService {
        ...
        try {
            ...
            await Promise.all([
                (async () => {
                    ...
                    if (existing) {
                        await this.db.from('UserStudyPreference').update(pref).eq('userId', user.id);
                    }
                    else {
                        await this.db.from('UserStudyPreference').insert(pref);
                    }
                })(),
                (async () => {
                    ...
                    if (existing) {
                        await this.db.from('UserAcademicProfile').update(prof).eq('userId', user.id);
                    }
                    else {
                        await this.db.from('UserAcademicProfile').insert(prof);
                    }
                })(),
                (async () => {
                    ...
                    if (existing) {
                        await this.db.from('UserFinancialProfile').update(fin).eq('userId', user.id);
                    }
                    else {
                        await this.db.from('UserFinancialProfile').insert(fin);
                    }
                })(),
            ]);
    ...
    async shareOnboardingLink(studentId, studentEmail, studentName, shareUrl, staffUser) {
        try {
