...
let ExploreService = class ExploreService {
    ...
    constructor(supabase, communityService, referenceService) {
    ...
    async getHubData(topic) {
        ...
        if (topic === 'loan')
            targetTopic = 'eligibility';
        const config = this.hubConfigs[targetTopic];
        if (!config)
            throw new common_1.NotFoundException(`Explore Hub for topic '${topic}' not found`);
